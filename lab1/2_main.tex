...
По теории МНК оценка $\hat{\theta}$ является состоятельной, несмещённой и эффективной при выполнении условий для помехи $v(k)$:
\begin{align}
    \mathbb{E}\{v(k)\} &= 0, &
    \mathbb{E}\{x_i(k) v(k)\} &= 0, \\
    \mathrm{Var}\{v(k)\} &= \sigma^2 = \text{const}, &
    \mathbb{E}\{v(k) v(k-\tau)\} &= 0,\quad \tau \neq 0.
\end{align}
Поскольку ошибка $e(k) = y(k) - \hat{y}(k)$ совпадает с оценкой помехи, анализ её статистических свойств позволяет судить о качестве модели и параметров: несмещенность соответствует нулевому среднему, отсутствие систематической корреляции с регрессорами и автокорреляции подтверждает эффективность и состоятельность оценки.
...
\subsection{Выводы}
Оцененные параметры: для \texttt{zad11} $\hat\theta=[-2.014,\ 1.999,\ 3.999]^\mathsf T$, для \texttt{zad12} $\hat\theta=[-1.953,\ 2.233,\ 3.575]^\mathsf T$.

Численные показатели свойств ошибки $e(k)$:
\begin{itemize}
    \item \textbf{zad11}:
    $\overline e \approx 0.0095$;\ 
    $\mathrm{Var}(e)\approx 0.906$;\ 
    $\mathrm{corr}(e,x_1)\approx -0.00$,\ $\mathrm{corr}(e,x_2)\approx -0.002$,\ $\mathrm{corr}(e,x_3)\approx 0.010$;\ 
    $\rho_e(1)\approx 0.013$.\ 
    Показатели близки к нулю, что согласуется с условиями $ \mathbb{E}\{v\}=0$, $ \mathbb{E}\{x_i v\}=0$ и отсутствием автокорреляции.
    \item \textbf{zad12}:
    $\overline e \approx 1.495$;\ 
    $\mathrm{Var}(e)\approx 3.187$;\ 
    $\mathrm{corr}(e,x_1)\approx 0.00$,\ $\mathrm{corr}(e,x_2)\approx -0.198$,\ $\mathrm{corr}(e,x_3)\approx 0.803$;\ 
    $\rho_e(1)\approx 0.716$.\ 
    Ненулевое среднее, высокая корреляция с $x_3$ и заметная автокорреляция указывают на нарушение предпосылок МНК и/или неполноту модели для этого набора.
\end{itemize}
...
\subsection{Выводы}
Суммы квадратов ошибок и свойства остатков:
\begin{itemize}
    \item \textbf{zad21}: $\mathrm{SSE}_{H1}=8488.56$, $\mathrm{SSE}_{H2}=301.75$.
    Для H1 среднее остатка $\overline e_{H1}\approx 1.2\cdot10^{-13}$, дисперсия $\mathrm{Var}(e_{H1})\approx 652.97$, автокорреляция лаг-1 $\rho_{H1}(1)\approx 0.807$, корреляция с температурой $\mathrm{corr}(e_{H1},T)\approx 0$.
    Для H2 $\overline e_{H2}\approx -7.9\cdot10^{-13}$, $\mathrm{Var}(e_{H2})\approx 23.21$, $\rho_{H2}(1)\approx -0.741$, $\mathrm{corr}(e_{H2},T)\approx 0$, $\mathrm{corr}(e_{H2},T^2)\approx 0$.
    Значительно меньшая $\mathrm{SSE}$ и дисперсия остатка у H2, а также отсутствие заметной корреляции с регрессорами позволяют признать квадратичную модель (H2) наиболее достоверной.
    \item \textbf{zad22}: $\mathrm{SSE}_{H1}=277.88$, $\mathrm{SSE}_{H2}=256.33$.
    Для H1: $\overline e_{H1}\approx -3.2\cdot10^{-14}$, $\mathrm{Var}(e_{H1})\approx 21.38$, $\rho_{H1}(1)\approx 0.001$, $\mathrm{corr}(e_{H1},T)\approx 0$.
    Для H2: $\overline e_{H2}\approx -8.4\cdot10^{-13}$, $\mathrm{Var}(e_{H2})\approx 19.72$, $\rho_{H2}(1)\approx -0.063$, $\mathrm{corr}(e_{H2},T)\approx 0$, $\mathrm{corr}(e_{H2},T^2)\approx 0$.
    Разница $\mathrm{SSE}$ невелика, но модель H2 обеспечивает немного меньшую дисперсию остатка без появления корреляций, поэтому её можно считать предпочтительной; при этом линейная модель также допустима.
\end{itemize}

Таким образом, для набора \texttt{zad21} достоверна квадратичная аппроксимация (H2), для \texttt{zad22} оба описания сопоставимы, однако H2 даёт чуть более точную аппроксимацию.
